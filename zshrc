...
function prompt_grml-large_precmd () {
    ...
    local -a left_items right_items
    left_items=(jobs history shell-level change-root time date newline
...
function grml_prompt_fallback () {
    ...
    p1="${BLUE}%n%m${NO_COLOR} %40<...<%B%~%b%<< "'${vcs_info_msg_0_}'"%# "
    if (( EUID == 0 )); then
...
if [[ "$TERM" == dumb ]] ; then
    ...
    for i in user path jobs history date time shell-level; do
    ...
    zstyle ':prompt:grml(|-large|-chroot):*:items:at' pre '%B%F{red}'
    zstyle ':prompt:grml(|-large|-chroot):*:items:host' pre '%B%F{red}'
else
    zstyle ':prompt:grml(|-large|-chroot):*:items:user' pre '%B%F{blue}'
    zstyle ':prompt:grml(|-large|-chroot):*:items:at' pre '%B%F{blue}'
    zstyle ':prompt:grml(|-large|-chroot):*:items:host' pre '%B%F{blue}'
