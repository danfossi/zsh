...
maxcolors=$(tput colors)

if [ $maxcolors -eq "8" ];then
    if [ "$EUID" -eq 0 ]; then
    ...
    if [ "$EUID" -eq 0 ]; then
        ...
        zstyle ':prompt:grml:*:items:at' pre '%B%F{red}'
        zstyle ':prompt:grml:*:items:host' pre '%B%F{red}'
        zstyle ':prompt:grml:*:items:percent' pre '%B%F{36}'
        zstyle ':prompt:grml:*:items:percent' post '%B%F{reset}'
        zstyle ':prompt:grml:right:items:sad-smiley' pre '%B%F{red}'
        zstyle ':prompt:grml:*:items:path' pre '%F{36}'
        zstyle ':prompt:grml:*:items:path' post '%F{reset}'
    else
        zstyle ':prompt:grml:*:items:user' pre '%B%F{69}'
        zstyle ':prompt:grml:*:items:at' pre '%B%F{69}'
        zstyle ':prompt:grml:*:items:host' pre '%B%F{69}'
        zstyle ':prompt:grml:*:items:percent' pre '%B%F{36}'
        zstyle ':prompt:grml:*:items:percent' post '%B%F{reset}'
        zstyle ':prompt:grml:right:items:sad-smiley' pre '%B%F{red}'
        zstyle ':prompt:grml:*:items:path' pre '%F{36}'
